# app/workers/train_p_freelancer_accept_visual.py
...
Output:
    ...
    - File ảnh: p_freelancer_accept_training_results.png
"""
...
def create_presentation_ready_visualization(model, X_test, y_test, y_pred, feature_cols, dataset_info):
    """Create presentation-ready visualization with large fonts and clear layout"""
    
    # Create figure with more space
    fig = plt.figure(figsize=(24, 18))
    
    # Main title
    fig.suptitle('🎯 p_freelancer_accept MODEL TRAINING RESULTS', 
    ...
    ax1.set_xticks([0, 1])
    ax1.set_yticks([0, 1])
    ax1.set_xticklabels(['DECLINED', 'ACCEPTED'], fontsize=16, fontweight='bold')
    ax1.set_yticklabels(['DECLINED', 'ACCEPTED'], fontsize=16, fontweight='bold')
...
📋 DISTRIBUTION:
   • Accepted: {dataset_info['positive']:,} ({dataset_info['pos_rate']:.1%})
   • Declined: {dataset_info['negative']:,} ({dataset_info['neg_rate']:.1%})
...
⚙️ MODEL CONFIG:
   ...
   • Class Weight: Balanced"""
    ...
    legend_elements = [
        Patch(facecolor='#4ECDC4', alpha=0.8, label='Positive Impact (↑ Accept Rate)'),
        Patch(facecolor='#FF6B6B', alpha=0.8, label='Negative Impact (↓ Accept Rate)')
    ]
    ...
    output_file = OUTPUT_DIR / "p_freelancer_accept_training_results.png"
...
async def main():
    ...
    print("🚀 TRAINING p_freelancer_accept MODEL WITH VISUALIZATION")
    print("=" * 60)
    ...
    top_features = create_presentation_ready_visualization(
    ...
    for i, (_, row) in enumerate(top_features.tail(5).iterrows(), 1):
        ...
        print(f"{i}. {row['feature'].replace('_', ' ').title():<30} {impact} Accept Rate")
