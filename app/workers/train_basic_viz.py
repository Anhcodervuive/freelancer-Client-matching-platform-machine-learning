# app/workers/train_basic_viz.py

"""
Basic training script với matplotlib thuần, không dùng seaborn để tránh lỗi compatibility.
"""
...
OUTPUT_DIR = Path("basic_viz")
OUTPUT_DIR.mkdir(exist_ok=True)


def create_basic_visualizations(model, X_test, y_test, y_pred, feature_cols):
    """Create basic visualizations using only matplotlib"""
    ...
    axes[0, 0].set_xticklabels(['Declined', 'Accepted'])
    axes[0, 0].set_yticklabels(['Declined', 'Accepted'])
    ...
    bars = axes[0, 1].barh(y_pos, feature_importance['importance'], color=colors)
    ...
    bars = axes[1, 0].bar(range(len(metrics)), list(metrics.values()), color=colors)
...
async def main():
    """Main training function with basic visualization"""
    print("🚀 Training with Basic Visualization (matplotlib only)...")
    ...
    create_basic_visualizations(model, X_test, y_test, y_pred, feature_cols)
